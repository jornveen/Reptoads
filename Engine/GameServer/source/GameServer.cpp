...
void tbsg::GameServer::Run()
{
	...
	while (cli.IsRunning())
	{
		ReceivePackets(); // TODO: Run this in a separate thread.
		HandlePackets();
...
void tbsg::GameServer::TestEventFunc(unsigned int a /*= 0*/)
{
...
void tbsg::GameServer::RunGameSimulation(ptl::vector<ptl::string>& allArguments)
{
	for (size_t i = 0; i < allArguments.size(); ++i)
	{
		...
		if (command == "--simulate-game")
		{
			...
			{
				// While argumentparsing the arguments have been checked with the following format: P1ID P2ID MonsterDeckName P1DeckName P2DeckName
