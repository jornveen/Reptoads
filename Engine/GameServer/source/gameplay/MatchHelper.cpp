...
//tbsg::MatchData tbsg::gameplay::CreateMatchData(ptl::vector<tbsg::Deck>& decksToUse, unsigned int playerOneID, unsigned int playerTwoID, unsigned int seed)
//{
//	return CreateMatchData(decksToUse[0], ptl::vector<tbsg::Deck>{decksToUse[1], decksToUse[2]}, ptl::vector<ptl::vector<Card*>>{}, ptl::vector<tbsg::Hero>{}, 2, 1, seed);
//}
	...
	unsigned int playerThatStarts, unsigned int seed)
{
	...
	//Fill the decks if they aren't empty
...
bool tbsg::gameplay::CreateContainer(ptl::vector<ptl::vector<unsigned int>>& toAddTowards, unsigned int amountToAdd /*= 0*/, unsigned int addUntilLimit /*= 0*/)
{
	if (amountToAdd == 0)
	{
		return false;
...
bool tbsg::gameplay::CreateContainer(ptl::vector<tbsg::Hero>& toAddTowards, unsigned int amountToAdd /*= 0*/, unsigned int addUntilLimit /*= 0*/)
{
	if (amountToAdd == 0)
	{
		return false;
	...
	//Old loop as the other way of initializing isn't supported..
...
bool tbsg::gameplay::FillContainer(ptl::vector<unsigned int>& toAddTowards, unsigned int amountToAdd, unsigned int addUntilLimit /*= 0*/)
{
	if (amountToAdd == 0)
	{
		return false;
...
bool tbsg::gameplay::FillContainer(ptl::vector<tbsg::Deck>& toAddTowards, unsigned int amountToAdd /*= 0*/, unsigned int addUntilLimit /*= 0*/)
{
	if (amountToAdd == 0)
	{
		return false;
...
tbsg::ResultOfRound tbsg::gameplay::OnPlay(tbsg::MatchData & matchData, scripting::LuaContext& luaContext, GameDataDatabase* gameDataModel)
{
    ...
    //Switch next player start for next round
    ...
    //cof::Info(("NextPlayerToStart: " + std::to_string(matchData.nextStartingPlayer)).c_str());
    ...
    for (auto currentPlayerId : playersID)
    {
		auto cardId = matchData.playedCards[currentPlayerId];
         if (cardId != 0)
        {
            //gameplay::RemoveCard(matchData.playerHands[currentPlayerId], matchData.playedCards[currentPlayerId]);
			 ...
			 auto itr =  std::find_if(matchData.playerHands[currentPlayerId].begin(), matchData.playerHands[currentPlayerId].end(),[&cardId](unsigned int id)
			{
			 ...
			 //of::Warn("DISCARD: Player({}) {}", currentPlayerId, cardId);
    ...
    //Bind functionality as an API towards Lua
    ...
    for (auto currentPlayerId : playersID)
    {
        ...
        //Check if player has submited the card
        ...
        if (isHeroDead[currentPlayer] == false && currentCardId != 0)
        {
            ...
            //give player reward from the card he has played
            //GiveReward(matchData.heroes[currentPlayer], gameDataModel->GetCard(currentCardId)->data.reward, currentResultOfRound, currentPlayer);

            //TODO: TO discard change adding maybe
        }

        //Death Check
        if (HandleDeath(isHeroDead, playersID.size(), matchData))
        {
            //TODO: Game end both players are dead.
	...
	if (isAtEndFight == false)
	{
		if (monsterDead == false)
		{
			...
			//increase the turn the player is in combat
		...
		else
		{
			...
			//Reset the player fight duration because the monster is dead.
