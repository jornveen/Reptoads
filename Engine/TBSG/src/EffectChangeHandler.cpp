...
void EffectChangeHandler::ProcessChanges(ptl::vector<tbsg::Change> changes, ptl::queue<tbsg::EffectEvent*>& effectEvent, ptl::shared_ptr<UIEventQueue> userInterface, tbsg::SceneHandler& sceneHandler, tbsg::ClientMatchState& clientMatchState, audio::AudioSystem& audioSystem, GameDatabase& database, gfx::Camera& camera, double& deltaTime,unsigned int playerIndex)
{
	...
	{
		//ptl::cout << "Change detected, index: " << count << " ,ID: " << static_cast<unsigned int>(change.changeType) << ptl::endl;
		switch (change.changeType)
		{
			case tbsg::EffectChange::Card_Health:
			{
				...
				//Audio depending on health up or down
				if (change.change > 0)
				{
					...
					//effectEvent.push(RegisterAPIEvent(&EffectChangeHandler::WaitForParticleEffect, "WaitForParticleEffect", &sceneHandler, particleID));
				}
				else
				{
					//Audio for health down
                    glm::vec3 position = sceneHandler.monsterSpawnPoints[sceneHandler.monsterSpawnPoints.size() - clientMatchState.monsterCards.size()]->transform.GetPos() + glm::vec3{ 0, 1, 0 };
			...
			case tbsg::EffectChange::Card_Armor:
			{
				...
				{
					effectEvent.push(RegisterAPIEvent(&EffectChangeHandler::PlaySoundClipName, "PlayAudioClip", "Monster1_utility-V1.wav", &audioSystem));
                    // TODO: Play the particle for getting armor PlayParticleEffect(&sceneHandler, sceneHandler.monsterSpawnPoints[sceneHandler.monsterSpawnPoints.size() - clientMatchState.monsterCards.size()]->transform.GetPos(), "DamageParticle.json");
                    glm::vec3 position = sceneHandler.monsterSpawnPoints[sceneHandler.monsterSpawnPoints.size() - clientMatchState.monsterCards.size()]->transform.GetPos() + glm::vec3{ 0, 1, 0 };
			...
			{
				//Audio playing on death monster
			...
			case tbsg::EffectChange::Deck_DiscardToDeck:
			{
				...
				{
					//for opponent can be added here if we want that in the future
				}
				break;
			}
...
bool EffectChangeHandler::MoveToNextPosition(tbsg::SceneHandler* sceneHandler, gfx::Camera* camera, double* deltaTime)
{
    ...
    {
        // Calculate lerp time based on lerp speed of player
    ...
    {
        // Calculate lerp speed based on the speed of the player to have the same time of arrival
    ...
    //waypoint::LerpTransform(sceneHandler->cameraWaypoints, *deltaTime, camera->m_Translation, camera->m_Rotation, lerpSpeedCamera);
    //waypoint::LerpTransform(sceneHandler->playerWaypoints, *deltaTime, sceneHandler->GetCurrentScene()->FindNodeByName("player")->transform, lerpSpeedPlayer);
    //waypoint::LerpTransform(sceneHandler->opponentWaypoints, *deltaTime, sceneHandler->GetCurrentScene()->FindNodeByName("opponent")->transform, lerpSpeedOpponent);
...
{
	// Jorn gave me permission to comment this out since it is unused and doesn't work for PS4 xoxo Hannes
   /* bool isRotationDone = waypoint::LerpRotation(sceneHandler->cardWaypoints, *deltaTime, sceneHandler->cardSpawnPoints[playerID][encounterID]->transform.GetRot(), speed);

    if (isRotationDone)
    {
        sceneHandler->cardWaypoints.alpha = 0;
        sceneHandler->cardWaypoints.currentWaypoint.x = 0;
        sceneHandler->cardWaypoints.currentWaypoint.y = 0;
        sceneHandler->cardWaypoints.isDone = false;
        return true;
    }*/
...
{
	/*
	const int cardIndex = sceneHandler.cardSpawnPoints[playerID].size() - clientMatchState.monsterCards.size();

    if (clearBeforeSpawn)
    {
        for (auto& child : sceneHandler.cardSpawnPoints[playerID][cardIndex]->GetChildren())
        {
            sceneHandler.DisableModel(child.get());
            child->Clear();
        }
    }

    scene::SceneNode* cardModel = sceneHandler.GetCardModelFromCache(cardId, playerID, playerID);
    if (cardModel != nullptr)
    {
        ...
        cardModel->transform.SetPos(glm::vec3(0,0,0));
        cardModel->transform.SetRot(glm::quat{ {-90,0,0} });
        cardModel->transform.SetScale(glm::vec3(0.5f, 0.5f, 0.5f));
    }
	*/
