	...
	gameEvents->appendListener(GameEvent::OpponentOnCardsSubmitted,
	                           [this](Packet&& packet)
	                           {
		                           ...
		                           for (unsigned int i = 0; i < amount; ++i)
		                           {
			                           ...
			                           //Spawing Cards needs to be debuged and checked
			                           if (cardID != 0)
			                           {
				                           CardSelectedVisual(1, cardID);
			                           }
			                           //TODO uiQueue->enqueue(UIinteraction::OnConfirmCard, &cardID);
		                           }
	                           });
...
{
	/*timeOutTimer.Update(deltaTime);

	if(timeOutTimer.timePassed > timerTick)
	{
		unsigned int timerTime = 30 - static_cast<unsigned int>(std::trunc(timeOutTimer.timePassed));
		uiQueue->enqueue(UIInteractionEvent::OnUpdateGameTimer, [timerTime]() mutable
			{
				return static_cast<void*>(&timerTime);
			});
...
void Game::OnGameStarted(Packet&& packet)
{
	...
	if (!sceneHandler->monsterSpawnPoints.empty())
	{
		for (int i = 0; i < clientMatchState->monsterCards.size(); ++i)
		{
			...
			{
				//TODO: change name to monster name that we use
	...
	//Setting up values for the card reveal
...
void Game::OpponentOnCardHover(Packet&& packet)
{
	...
	//TODO	uiQueue->enqueue(UIinteraction::OnHoverOverCard, &cardID);
}
...
void Game::OpponentOnCardSelected(Packet&& packet)
{
	...
	//TODO uiQueue->enqueue(UIinteraction::OnSelectCard, &cardID);
}
...
void Game::ProcessChanges(Packet&& packet)
{
	...
	//if (playedCards[1] != 0)
	//{
	//    effectEvents.push(EffectChangeHandler::RegisterAPIEvent(&EffectChangeHandler::RevealCard, "RevealCard", sceneHandler, 1, sceneHandler->cardSpawnPoints[1].size() - clientMatchState->monsterCards.size(), 10, deltaTime));
	//}
...
void Game::LoadProvidedMapID(Packet&& packet)
{
	...
	//packet >> mapID;
	//TODO replace this with the ID sent over from the server
	sceneHandler->LoadMap(tbsg::HashString("FrogLander"));
}
...
void Game::CardSelectedVisual(unsigned playerIndex, unsigned cardId)
{
	...
	const auto spawnPointIndex = sceneHandler->cardSpawnPoints[playerID].size() - clientMatchState->monsterCards.size();
