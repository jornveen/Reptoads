...
{
    /**
     * \brief generates a renderble ID which is internally used to identify models: For checking if bound, to bind to the renderer.
     * \param sceneIndex the index of the current scene
     * \param renderableName the name of the renderble
     * \param nodeName the name of the node
     * \return a new renderble ID
     */
...
void tbsg::SceneHandler::Initialize()
{
...
#else
    FILE* fp = fopen(tbsg::Config::Get().mapsFile.c_str(), "r"); // non-Windows use "r"
    if (!fp)
    ...
    m_cachedCardUnitsOfPlayers.push_back({});//p1
    m_cachedCardUnitsOfPlayers.push_back({});//p2
}
...
void tbsg::SceneHandler::DisableScene(unsigned sceneIndex)
{
    ...
    for (auto& sceneNode : *m_scenes[sceneIndex]) {
        for (auto& renderable : sceneNode.renderables) {
            //std::cout << renderable->GetId() << "ID: " << renderable->id <<  " NAME: "<< ("Renderable " + ptl::to_string(++counter)).c_str() << std::endl;
            //m_view->UnbindRenderable(("Renderable " + ptl::to_string(++counter)).c_str());
            //m_view->UnbindRenderable(MakeRenderableID(sceneIndex, renderable->GetId(), sceneNode.name).c_str());
...
{
    auto& rm = m_view->GetGfxResourceManager();
    //auto& pass = rm.GetRenderPass("standard");
    for (auto& sceneNode : *m_scenes[sceneIndex]) 
    {
        for (auto& renderable : sceneNode.renderables) 
        {
            //m_view->BindRenderable(*renderable, pass, MakeRenderableID(sceneIndex, renderable->GetId(), sceneNode.name).c_str());
...
void tbsg::SceneHandler::UnloadScene(unsigned int sceneID)
{
    ...
    for (auto& sceneNode : *m_scenes[sceneID]) {
        for (auto& renderable : sceneNode.renderables) {
			//m_view->RemoveRenderable(renderable.renderableRegisteredId);
            // m_view->RemoveRenderable(("Renderable " + ptl::to_string(++counter)).c_str());
        }
    }
}
...
void tbsg::SceneHandler::LoadParticles()
{
    ...
    ASSERT_MSG(!err, "Could not load config");
#else
    fp = fopen(particleListPath.c_str(), "r"); // non-Windows use "r"
    if (!fp)
        ASSERT_MSG(false, "Could not load config");
#endif
...
scene::SceneNode* tbsg::SceneHandler::AddModel(ptl::string name, core::Transform& transform, scene::SceneNode* parent)
{
    ...
    //auto& rm = m_view->GetGfxResourceManager();
    //auto& pass = rm.GetRenderPass("standard");
	/*auto* child =  parent->FindNodeByName(name);*/
    auto it = loadedModels.find(name);
    if(it != loadedModels.end() /*&& child == nullptr*/)
    {
        ...
        //m_view->BindRenderable(*nodeRenderable, pass, MakeRenderableID(m_currentScene, nodeRenderable->GetId(), name).c_str());
		//RenderableId renderableId = m_view->AddRenderable(&sceneNode->transform, meshId, materialId);
        return copyModel;
    }
...
void tbsg::SceneHandler::DisableModel(scene::SceneNode* model)
{
    ...
    for (auto& renderable : model->renderables){
        ...
        // m_view->UnbindRenderable(renderable);
}
    ...
    core::Transform localTransform, scene::SceneNode* parent, bool reverseWindingOrder, bool flipXAxisUVs)
{
	...
	//TODO: Cache material usage. 
    /*if (rm.ContainsMaterial(materialId.c_str())) {
        material = &rm.GetMaterial(materialId.c_str());
    }
    else */
	//{
    ...
    //ptl::string renderableBindId = "Worldspace plane renderable bound: #" + ptl::to_string(currentCount);
    //m_view->BindRenderable(renderable, renderPass, MakeRenderableID(m_currentScene, renderable.GetId(), parent->name).c_str());
                                                     ...
                                                     core::Transform transform, scene::SceneNode* parent)
{
	...
	//cubeRenderable.renderableRegisteredId = m_view->AddRenderable(&node->transform, cubeRenderable.meshId, cubeRenderable.materialId);
...
scene::SceneNode* tbsg::SceneHandler::CreateCardModelForPlayer(unsigned cardID, unsigned playerIndex)
{
    ...
    if (result != cardUnitCacheOfPlayer.end()) return result->second.get();
	*/
