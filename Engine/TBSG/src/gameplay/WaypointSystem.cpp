...
namespace waypoint
{
    ...
    /**
     * \brief This function lerps the rotation of the given rotation over the waypoints
     * \param waypointData
     * \param deltaTime
     * \param rotation
     * \param speed
    ...
    /**
     * \brief This function lerps both the position and rotation of the given transform over the waypoints
     * \param WaypointData& waypointData
     * \param const float& deltaTime
     * \param core::Transform& transform
     * \param float speed
     * \return bool
     */
        ...
        float speed)
    {
        ...
        // Check what the transform of the waypoint before is
        ...
        // Check what the target transform is
        if (waypointData.currentWaypoint.y + 1 >= waypointData.waypoints[waypointData.currentWaypoint.x].size())
        {
            afterWaypointY = 0;
            ++afterWaypointX;
        ...
        // Check what the after transform is
        ...
        //const float epsilon = 0.1f; // DEBUG
        ...
        transform.SetRot(glm::slerp(startingTransform.GetRot(), targetTransform.GetRot(), waypointData.alpha));
        //transform.SetRot(glm::normalize(glm::catmullRom(beforeTransform.GetRot(), startingTransform.GetRot(), targetTransform.GetRot(), afterTransform.GetRot(), waypointData.alpha)));
