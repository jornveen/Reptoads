...
namespace TextureUtil
{
	...
	GnfError loadGnfHeader(sce::Gnf::Header *outHeader, FILE *gnfFile)
	{
		...
		if (fseek(gnfFile, 0, SEEK_SET) != 0)
		{
			return TextureUtil::kGnfErrorCorruptHeader;
		}
		if (fread(outHeader, sizeof(sce::Gnf::Header), 1, gnfFile) != 1)
		{
			return TextureUtil::kGnfErrorCorruptHeader;
		}
	...
	*/
	GnfError readGnfContents(sce::Gnf::Contents *outContents, uint32_t contentsSizeInBytes, FILE *gnfFile)
	{
		...
		if (fseek(gnfFile, sizeof(sce::Gnf::Header), SEEK_SET) != 0)
		{
			return TextureUtil::kGnfErrorCouldNotOpenFile;
		}
		...
		sce::Gnmx::Toolkit::Allocators &allocators)
	{
		...
		do
		{
			...
			if (fseek(gnfFile, sizeof(sce::Gnf::Header) + header.m_contentsSize + getTexturePixelsByteOffset(gnfContents, textureIndex), SEEK_SET) != 0)
			{
				result = kGnfErrorCouldNotOpenFile;
				break;
			}
			if (fread(pixelsAddr, pixelsSa.m_size, 1, gnfFile) != 1)
			{
				result = kGnfErrorFileIsTooShort;
				break;
			}
