...
// 	explicit PunctualLight(LightDist a_maxLightDistance);
// 	explicit PunctualLight(float a_ca, float a_la, float a_qa);
// 	void SetAttenuation(LightDist a_maxLightDistance);
// 	virtual void SetConstantAttenuation(const float a_la) { m_linearAttenuation = a_la; }
// 	virtual void SetLinearAttenuation(const float a_la) { m_linearAttenuation = a_la; }
// 	virtual void SetQuadraticAttenuation(const float a_qa) { m_quadraticAttenuation = a_qa; }
...
/// Concept like syntax emulated with macro's in C++17
...
/// A static_assert based approach for defining the PointLight concept
