...
namespace gfx
{
	...
	public:
		...
		/** 
		 *	Create a GPU texture from a byte array.
		 *	
		 *  \Param id = the identifier you use to retrieve the texture again from GetTexture(id)
		 *  \Param texBuffer = the byte array where the raw texture data is stored.
		 *  \Param width = the width of the texture in pixels
		 *  \Param height = the height of the texture in pixels
		 *  \Param bitsPerChannel = the amount of bits(eg. 8 bits is a char) per color channel(usually r,g,b,a so 4)
		 *  \Param amountOfChannels = the amount of color channels(usually r,g,b,a so 4)
		 *  \Param optionalName = a name which will be given to the resource in DX12
		 */
		...
		/// `shaderPath` should automatically get prefixed with `tbsg::Config::Get().GetShadersFolder()`
		...
		/// Get the first texture that has been loaded from the path \texturePath
		/// \NOTE	There could be multiple textures registered to texturePath (in the case it came from a .gltf for example).
		///			In that case only the first texture is returned.
		...
		/// Check if any texture has been loaded from the path \texturePath
		/// \NOTE There could be multiple textures registered to texturePath (in the case it came from a .gltf for example)
