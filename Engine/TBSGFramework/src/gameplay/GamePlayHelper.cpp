...
ptl::vector<unsigned int> tbsg::gameplay::DrawCards(ptl::vector<unsigned int>& toDrawFrom, ptl::vector<unsigned int>& toPlaceIn, unsigned int amountToDraw, unsigned int drawUntilLimit)
{
	...
	else
	{
        if (drawUntilLimit != 0)
        {
            // if amount of cards exceed total limit, adjust amount to draw to draw until limit
            if ((toPlaceIn.size() + amountToDraw) > drawUntilLimit) {
                amountToDraw = drawUntilLimit - static_cast<std::uint32_t>(toPlaceIn.size());
            }
        }

		//if amount of cards in 'toDrawFrom' isn't enough to supply the request, change it to size.
		if (toDrawFrom.size() < amountToDraw) {
			amountToDraw = static_cast<std::uint32_t>(toDrawFrom.size());
		}
		//move cards to new location, erase old.
...
ptl::vector<unsigned int> tbsg::gameplay::DrawCards(ptl::vector<tbsg::Card*>& toDrawFrom, ptl::vector<tbsg::Card*>& toPlaceIn, unsigned int amountToDraw /*= 0*/, unsigned int drawUntilLimit /*= 0*/)
{
	ptl::vector<unsigned int> cardsDrawn{};
	if (amountToDraw == 0) {
		return cardsDrawn;
