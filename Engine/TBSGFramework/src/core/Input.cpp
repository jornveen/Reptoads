...
namespace tbsg {
	...
	Input::Input(int refWinSizeX, int refWinSizeY) : m_RefWinSizeX(refWinSizeX), m_RefWinSizeY(refWinSizeY)
	{
		...
		m_handle = scePadOpen(userId,
			SCE_PAD_PORT_TYPE_STANDARD, 0, NULL);
...
#ifdef PLATFORM_PS
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_UP, InputType::EGamepad_DPad_Up);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_DOWN, InputType::EGamepad_DPad_Down);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_LEFT, InputType::EGamepad_DPad_Left);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_RIGHT, InputType::EGamepad_DPad_Right);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_OPTIONS, InputType::EGamepad_Special_Right);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_TOUCH_PAD, InputType::EGamepad_Special_Left);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_L3, InputType::EGamepad_Left_Thumb_Button);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_R3, InputType::EGamepad_Right_Thumb_Button);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_L1, InputType::EGamepad_Left_Shoulder);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_R1, InputType::EGamepad_Right_Shoulder);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_CROSS, InputType::EGamepad_Face_Button_Down);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_CIRCLE, InputType::EGamepad_Face_Button_Right);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_SQUARE, InputType::EGamepad_Face_Button_Left);
		m_ControllerMapping.emplace(SCE_PAD_BUTTON_TRIANGLE, InputType::EGamepad_Face_Button_Up);
	...
	void Input::HandleController()
	{
		if (m_handle >= 0) {
			...
			ret = scePadReadState(m_handle, &data);
			if (ret == 0) {
				for (auto mapping : m_ControllerMapping) {
					const bool pressed = ((data.buttons & mapping.first) != 0);
				...
				const float normLX = static_cast<float>(data.leftStick.x) / 255.0f * 2.0f - 1.0f;
				const float normLY = (static_cast<float>(data.leftStick.y) / 255.0f * 2.0f - 1.0f) * -1.0f;
				...
				const float normRX = static_cast<float>(data.rightStick.x) / 255.0f * 2.0f - 1.0f;
				const float normRY = (static_cast<float>(data.rightStick.y) / 255.0f * 2.0f - 1.0f) * -1.0f;
				...
				const float leftTrigger = static_cast<float>(data.analogButtons.l2) / 255;
				const float rightTrigger = static_cast<float>(data.analogButtons.r2) / 255;
	...
	void Input::HandleKeyboardDialog()
	{
		...
		if (dialogOpened)
		{
			...
			if (res != SCE_OK)
			{
				printf("ERROR: sceImeDialogGetResult: res=%08x\n", res);
				return;
			}

			if (dialogResult.endstatus == SCE_IME_DIALOG_END_STATUS_ABORTED)
			{
				printf("Aborted IME Dialog input.\n");
			}
			else if (dialogResult.endstatus == SCE_IME_DIALOG_END_STATUS_USER_CANCELED)
			{
				printf("User canceled IME Dialog input.\n");
			}
			else
			{
				printf("Terminate IME Dialog input.\n");
			}

			res = sceImeDialogTerm();
			if (res != SCE_OK)
			{
				printf("ERROR: sceImeDialogTerm: res=%08x\n", res);
				return;
			}
			printf("dialogResult.endstatus = %d\n", dialogResult.endstatus);
			dialogOpened = false;
			
			ptl::string output(dialogTextBuffer.begin(), dialogTextBuffer.begin() + dialogTextBuffer.find(L'\0'));
	...
	void Input::OpenInputDialog(bool obsecure, ptl::string currentText)
	{
		...
		if (!dialogOpened)
		{
			...
			dialogTextBuffer.clear();
			dialogTextBuffer = ptl::wstring(currentText.begin(), currentText.end());
			dialogTextBuffer.resize(1024);

			dialogParam.userId = userId;
			dialogParam.type = obsecure ? SCE_IME_TYPE_BASIC_LATIN : SCE_IME_TYPE_DEFAULT;
			dialogParam.option = obsecure ? (SCE_IME_OPTION_DEFAULT | SCE_IME_OPTION_PASSWORD) : SCE_IME_OPTION_DEFAULT;
			dialogParam.supportedLanguages = 0;
			dialogParam.enterLabel = SCE_IME_ENTER_LABEL_DEFAULT;
			dialogParam.inputMethod = SCE_IME_INPUT_METHOD_DEFAULT;
			dialogParam.title = nullptr;
			dialogParam.placeholder = nullptr;
			dialogParam.inputTextBuffer = &dialogTextBuffer[0];
			dialogParam.maxTextLength = 1024;

			uint32_t dialogWidth, dialogHeight;
			sceImeDialogGetPanelSizeExtended(&dialogParam, NULL, &dialogWidth, &dialogHeight);
			printf("    # Dialog Size(%d,%d)\n", dialogWidth, dialogHeight);

			dialogParam.posx = 1920.0f / 2.0f - dialogWidth / 2.0;
			dialogParam.posy = 1080.0f / 2.0f - dialogHeight / 2.0;
			dialogParam.horizontalAlignment = SCE_IME_HALIGN_LEFT;
			dialogParam.verticalAlignment = SCE_IME_VALIGN_TOP;
			dialogParam.filter = NULL;
