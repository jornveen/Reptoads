...
namespace gfx
{
	...
	void Camera::Translate(glm::vec3 translation, Space space)
	{
		...
		//m_Translation = XMVectorSetW(pData->m_Translation, 1.0f);
	...
	{
		m_ProjectionMatrix = glm::perspective(glm::radians(m_vFoV), m_AspectRatio, m_zNear, m_zFar); //XMMatrixPerspectiveFovLH(XMConvertToRadians(m_vFoV), m_AspectRatio, m_zNear, m_zFar);
