...
namespace gfx
{
	...
	{
		// It can happen that an uninitialized emitter is removed
	...
	void IParticleRenderer::InitializeParticleEmitter(ParticleEmitter& emitter, float totalTime, float deltaTime)
	{
		...
		for (uint32_t i = 0; i < initialParticleAmount; ++i) {
			...
			// 	emitter.startParameters.simulationSpace == SimulationSpace::LocalOnlyPosition) {
			glm::vec3 relativeOffset = emitter.startParameters.spawnOffset.Value();
			//XMStoreFloat3(&particlePos, XMVector3TransformCoord(relativeOffset, transform.CalculateModelMatrix()));
			particlePos = emitter.particleEmitterPosition + relativeOffset;

			particleVel = emitter.startParameters.initialVelocity.Value();

			// } else if (emitter.startParameters.simulationSpace == SimulationSpace::World) {
				// XMStoreFloat3(&particlePos, emitter.startParameters.spawnOffset.Value());
				//
				// XMStoreFloat3(&particleVel, emitter.startParameters.initialVelocity.Value());
			// }
		...
		for (int i = static_cast<int>(particles.size() - 1); i >= 0; --i) {
			...
			if (lifeTime > 0.0f) {
				if (particle.birthTime + lifeTime <= totalTime) {
					//swap so we erase the last element and don't move all the particles
