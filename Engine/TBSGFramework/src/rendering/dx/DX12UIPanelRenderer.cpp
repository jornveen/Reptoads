...
namespace gfx
{
	...
	void DX12UIPanelRenderer::StartFrame()
	{
		...
		// XMMATRIX ortho = XMMatrixOrthographicLH(renderer.m_Viewport.Width, renderer.m_Viewport.Height, 0, 100);
	...
	void DX12UIPanelRenderer::RenderPanel(gfx::IUIRenderer::CachedUIText* uiPanel)
	{
		...
		//auto& tex = renderer->GetGfxResourceManager().GetTexture(uiImage->GetTextTextureId());
	...
	void DX12UIPanelRenderer::AddImage(gfx::IUIRenderer::CachedUIImage& uiImage)
	{
		...
		// commandList->CopyVertexBuffer(vtxBuffer, uiImage.vertices.size(), sizeof(gfx::Vertex), uiImage.vertices.data());
		// auto fence = commandQueue->ExecuteCommandList(commandList);
		// commandQueue->WaitForFenceValue(fence);
		//
		// uiImageCache[uiImage.uiImage] = std::move(vtxBuffer);
	}
	...
	void DX12UIPanelRenderer::UpdateImage(gfx::IUIRenderer::CachedUIImage& uiImage)
	{
		...
		// auto it = uiImageCache.find(uiImage.uiImage);
		// ASSERT(it != uiImageCache.end());
		//
	...
	void DX12UIPanelRenderer::RemoveImage(ui::UIImage& uiImage)
	{
		...
		// auto it = uiImageCache.find(&uiImage);
		// ASSERT(it != uiImageCache.end());
		// uiImageCache.erase(it);
	}

	/*std::array<DX12UIPanelRenderer::UIVertex, 4> DX12UIPanelRenderer::CalculatePanelQuad(UIPanel* panel, glm::vec2 additionalOffset)
	{
		...
		for (int i = 0; i < 4; ++i) {
			UIVertex vert{};
			glm::vec2 vertpos = panel->pos + additionalOffset + dirs[i] * panel->size;
			glm::vec2 vertuv = dirs[i] * dirToUV;
			vert.Position = glm::vec3(vertpos.x, vertpos.y, panel->zDepth);
