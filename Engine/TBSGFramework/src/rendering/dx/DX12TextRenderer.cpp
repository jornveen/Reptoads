...
namespace gfx
{
	...
	//The references I tried to use primarily:
	//https://docs.microsoft.com/en-us/windows/desktop/direct3d12/d2d-using-d3d11on12
	//https://stackoverflow.com/questions/21188593/d3d-d2d-interop-on-windows-7
	void DX12TextRenderer::Initialize()
	{
		...
		// Create D2D/DWrite components.
		{
...

			{
				...
				for (auto& p : ghc::filesystem::recursive_directory_iterator{ fontPath }) {
					if (p.is_regular_file()) {
						...
						loadFontFileResult = m_dWriteFactory->CreateFontFileReference(ptl::transform_to<ptl::wstring>(p.path().string()).c_str(), /* lastWriteTime*/ nullptr, &pFontFile);
			...
			//IDWriteFontFile* pFontFile;
			//hr = m_dWriteFactory->CreateFontFileReference(L"data/fonts/ZCOOLKuaiLe-Regular.ttf", /* lastWriteTime*/ nullptr, &pFontFile);
			//ThrowIfFailed(hr);
		...
		//	// colorBuffer.GetD3D12Resource().Get(),
		//	renderer_->GetRenderTarget().GetTexture(AttachmentPoint::Color0).GetD3D12Resource().Get(),
		//	&d3d11Flags,
		//	D3D12_RESOURCE_STATE_RENDER_TARGET,
		//	D3D12_RESOURCE_STATE_RENDER_TARGET,
		//	IID_PPV_ARGS(&m_wrappedColorBuffer)
		//));

		//// Create a render target for D2D to draw directly to this back buffer.
		//Microsoft::WRL::ComPtr<IDXGISurface> surface;
		//ThrowIfFailed(m_wrappedColorBuffer.As(&surface));
		...
		if (FAILED(D3DCompileFromFile(L"data/shaders/UIOverlayShader.VS.hlsl", nullptr, nullptr, /*nullptr*/"main", "vs_5_1", D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &particleVertexShaderBlob, &errororor))) {
		...
		if (FAILED(D3DCompileFromFile(L"data/shaders/UIOverlayShader.PS.hlsl", nullptr, nullptr, /*nullptr*/"main", "ps_5_1", D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &particlePixelShaderBlob, &errororor))) {
			ASSERT(errororor != nullptr);
			auto* cstr = static_cast<const char*>(errororor->GetBufferPointer());
		...
		//From: https://github.com/Microsoft/DirectX-Graphics-Samples/blob/07008938a0dc5a187a23abcb55b61f8c2809c874/MiniEngine/Core/GraphicsCommon.cpp#L173-L195
		//TODO: HACK this blend state is not correct, but the end result looks surprisingly good
	  ...
	  //   alphaBlend.RenderTarget[0].DestBlend = D3D12_BLEND_ONE;
		 //alphaBlend.RenderTarget[0].SrcBlend = D3D12_BLEND_SRC_ALPHA;
		 //CD3DX12_BLEND_DESC blendDesc(alphaBlend);
	...
	void DX12TextRenderer::StartFrame()
	{
		...
		// m_d3d11On12Device->AcquireWrappedResources(m_wrappedColorBuffer.GetAddressOf(), 1);
		//
	...
	void DX12TextRenderer::RenderUIText(IUIRenderer::CachedUIText& cachedUiText)
	{
		...
		//auto& text = renderer->GetGfxResourceManager().GetTexture(uiImage->GetImagePath().c_str());
		commandList_->SetShaderResourceView(1, 0, *textTexture->dx12Texture, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);

		// auto vertices = CalculatePanelQuad(uiPanel, additionalOffset);
		...
		// commandList_->SetDynamicVertexBuffer(0, vertices.size(), sizeof(UIVertex), vertices.data());
		// commandList_->SetDynamicIndexBuffer(quadIndices_.size(), DXGI_FORMAT_R32_UINT, quadIndices_.data());
		commandList_->DrawIndexed(static_cast<uint32_t>(cachedUiText.indices.size()));

		// float viewportHeight = renderer_->GetViewport().Height;
		// glm::vec2 pos = uiText->pos + additionalOffset;
		//
		// D2D1_RECT_F d2dRect = { pos.x, viewportHeight - (pos.y + uiText->size.y), pos.x + uiText->size.x,  viewportHeight - pos.y };
		//
		// std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
		// std::wstring wide = converter.from_bytes(uiText->text.c_str());
		//
		// m_textFormat->SetTextAlignment(GetHorizontalTextAlignmentFromComponent(uiText->textAlignment));
		// m_textFormat->SetParagraphAlignment(GetVerticalTextAlignmentFromComponetn(uiText->verticalAlignment));
		//
	...
	void DX12TextRenderer::EndFrame()
	{
		...
		// m_d3d11On12Device->ReleaseWrappedResources(m_wrappedColorBuffer.GetAddressOf(), 1);
		//
	...
	auto DX12TextRenderer::CreateTextureFromText(const gfx::UIText& uiText) -> TextTexture&
	{
		...
		UITextOptionsForTexture options{};// { uiText };
	...
	auto DX12TextRenderer::CreateTextureFromText(const UITextOptionsForTexture& uiText) -> TextTexture&
	{
		...
		D3D12_RESOURCE_DESC textureDesc = CD3DX12_RESOURCE_DESC::Tex2D(backbufferDesc.Format, static_cast<UINT64>(size.x), static_cast<UINT>(size.y), 1, 1, 1, 0,
			D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET/*D3D12_RESOURCE_FLAG_ALLOW_CROSS_ADAPTER*/, D3D12_TEXTURE_LAYOUT_UNKNOWN/*D3D12_TEXTURE_LAYOUT_ROW_MAJOR*/, 65536);
		...
		ThrowIfFailed(device->CreateCommittedResource(&heapProperties,
			D3D12_HEAP_FLAG_NONE /*D3D12_HEAP_FLAG_SHARED_CROSS_ADAPTER | D3D12_HEAP_FLAG_SHARED*/,
			&textureDesc,
			D3D12_RESOURCE_STATE_RENDER_TARGET,
			nullptr,
		...
		//d3d11Flags.MiscFlags = D3D11_RESOURCE_MISC_SHARED;
		ThrowIfFailed(m_d3d11On12Device->CreateWrappedResource(
			textureResource.Get(),
			&d3d11Flags,
			D3D12_RESOURCE_STATE_RENDER_TARGET,
		...
		// ==== Now draw the text to the texture ====
		PushD2DWarningFilter();
		...
		//TextureId placedTextureId = renderer_->GetGfxResourceManager().AddTexture(std::move(texture));
	...
	void DX12TextRenderer::AddUIText(ui::UIText* uiText)
	{
		...
		// add the textureId to the uiText instance to obtain the minUVs
	...
	void DX12TextRenderer::RemoveUIText(ui::UIText& uiText)
	{
		...
		if (it != textCache_.end()) {
			////it->second.decrease_ref_count();
			////if (it->second.ref_count() <= 0) {
			////	RemoveTextTexture(it->second.value().dx12Texture);
			////	textCache_.erase(it);
			////}
