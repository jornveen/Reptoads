...
void ResourceStateTracker::CommitFinalResourceStates()
{
    ...
    for (const auto& resourceState : m_FinalResourceState)
    {
		...
		{
			//No complicated subresource states, we can just set the state
			globalResourceState = resourceState.second;
		} 
    	else
		{
			//The subresource has different state then the whole resource, we need to merge.


			if (globalResourceState.State == resourceState.second.State) {
				//TODO: check which side is bigger, for optimization
