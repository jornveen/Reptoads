...
namespace gfx
{
	...
	, bloomTex(&bloomTexture), bloomTexFormat(bloomTextureFormat)
	{
		...
		// alphaBlend.RenderTarget[0].DestBlendAlpha = D3D12_BLEND_ONE;
		...
		for (uint32_t i = 0; i < static_cast<uint32_t>(emitter._particles.size()); ++i) {
			...
			for (uint32_t j = 0; j < 4; ++j) {
				...
				if (emitter.startParameters.billboardType == BillboardType::WorldspaceQuad) {
					...
					//glm::vec3 CameraRight_worldspace = XMVectorSet(cpuViewMat.m[0][0], cpuViewMat.m[1][0], cpuViewMat.m[2][0], 1.0f);
				...
				//Convert [-1,1] range to [0, 1] and then invert the y
