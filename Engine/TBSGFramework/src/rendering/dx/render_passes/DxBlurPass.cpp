...
namespace gfx
{
	...
	void DxBlurPass::ExecutePass(Camera& camera, void* perFrameResources)
	{
		...
		for(uint32_t i = 0; i < mipLevels; ++i) {
			...
			/*{
				auto it = ResourceStateTracker::ms_GlobalResourceState.find(mippedTextureToBlur.GetD3D12Resource().Get());
				if (it != ResourceStateTracker::ms_GlobalResourceState.end()) {
					for (auto& subresourceAndStatePair : it->second.SubresourceState) {
						auto subresource = subresourceAndStatePair.first;
						auto state = subresourceAndStatePair.second;

						commandList->TransitionBarrier(it->first, it->second.State, subresource);
					}
				}
			}
			{
				auto it = ResourceStateTracker::ms_GlobalResourceState.find(horizontalBlurredTexture.GetD3D12Resource().Get());
				if (it != ResourceStateTracker::ms_GlobalResourceState.end()) {
					for (auto& subresourceAndStatePair : it->second.SubresourceState) {
						auto subresource = subresourceAndStatePair.first;
						auto state = subresourceAndStatePair.second;
			...
			//commandList->Dispatch(width/8+1, height/8+1);
		}

		//blurData.blurHorizontal = false;
		//for (uint32_t i = 0; i < mipLevels; ++i) {
		//	blurData.mipLevel = i;
		//	int width = glm::max<uint32_t>(1, texWidth >> i);
		//	int height = glm::max<uint32_t>(1, texHeight >> i);
		...
		//	commandList->SetComputeDynamicConstantBuffer(0, blurData);
		//	commandList->SetShaderResourceView(1, 0, horizontalBlurredTexture, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, 0, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
		//	commandList->SetUnorderedAccessView(1, 1, mippedTextureToBlur, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, 0, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, &uavDesc);
		//	commandList->Dispatch(width / 8 + 1, height / 8 + 1);
		//}
