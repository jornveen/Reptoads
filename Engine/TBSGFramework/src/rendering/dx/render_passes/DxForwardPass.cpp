...
namespace gfx
{
	...
	{
		PerFrameConstants,	// SHADER_cbuffer PerFrameConstantsVs SHADER_CONSTANT_REGISTER(0)
		PerObjectConstants,	// SHADER_cbuffer PerObjectConstantsVs SHADER_CONSTANT_REGISTER(1)
		Material,			// SHADER_cbuffer ShaderMaterial SHADER_CONSTANT_REGISTER(2)
		Textures,				// Texture2D<float4> diffuseTexture : register(t0);
								// Texture2D<float3> emissiveTexture : register(t1);
								// RWTexture2D<float4> bloomTexture SHADER_UAV_REGISTER(0);
		NumRootParameters
	};
	...
	{ BROFILER_CATEGORY("DxForwardPass::ExecutePass", Profiler::Color::Cyan)
		...
		for (auto& renderable : renderer->renderables) {
			...
			//commandList->SetUnorderedAccessView(ForwardRootParameters::Textures, 2, rwBloomTexture);
			...
			//BROFILER_INLINE_EVENT("SetShaderResourceView Texture", commandList->SetShaderResourceView(RootParameters::TexturesEtc, 0, diffuseTexture, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE); );
