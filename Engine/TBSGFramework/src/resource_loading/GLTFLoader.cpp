	...
	if (it == renderableMeshMap_.end()) {
		...
		for (auto primitiveIndex = 0; primitiveIndex < mesh.primitives.size(); ++primitiveIndex) {
			...
			// Add vertex to Vertex buffer...
			{
				...
				// const auto vertexCount = positionAccesor.count;
			...
			MaterialId materialId{0}; // default to first material created
...
MaterialId GLTFLoader::CreateMaterial(int materialIndex)
{
	...
	//if (
	//	extras.Has("specular_power") &&
	//	extras.Has("ambient_color") &&
	//	extras.Has("diffuse_color") &&
	//	extras.Has("specular_color")
	//	) {
	//	emissive = {
	//		static_cast<float>(extras.Get("emissiv_color").Get<tinygltf::Value::Array>()[0].Get<double>()),
	//		static_cast<float>(extras.Get("emissiv_color").Get<tinygltf::Value::Array>()[1].Get<double>()),
	//		static_cast<float>(extras.Get("emissiv_color").Get<tinygltf::Value::Array>()[2].Get<double>())
	//	};
	//	ambient = {
	//static_cast<float>(extras.Get("ambient_color").Get<tinygltf::Value::Array>()[0].Get<double>()),
	//static_cast<float>(extras.Get("ambient_color").Get<tinygltf::Value::Array>()[1].Get<double>()),
	//static_cast<float>(extras.Get("ambient_color").Get<tinygltf::Value::Array>()[2].Get<double>())
	//	};
	//	diffuse = {
	//static_cast<float>(extras.Get("diffuse_color").Get<tinygltf::Value::Array>()[0].Get<double>()),
	//static_cast<float>(extras.Get("diffuse_color").Get<tinygltf::Value::Array>()[1].Get<double>()),
	//static_cast<float>(extras.Get("diffuse_color").Get<tinygltf::Value::Array>()[2].Get<double>())
	//	};
	//	specular = {
	//static_cast<float>(extras.Get("specular_color").Get<tinygltf::Value::Array>()[0].Get<double>()),
	//static_cast<float>(extras.Get("specular_color").Get<tinygltf::Value::Array>()[1].Get<double>()),
	//static_cast<float>(extras.Get("specular_color").Get<tinygltf::Value::Array>()[2].Get<double>())
	//	};
	//
	//	specularPower = extras.Get("specular_power").Get<double>();
	//
	//} else {
	//	//default:
	...
	if (diffuseTextureIndex != -1) { //The material has a texture...
...
#ifdef PLATFORM_PS // this crashes on PC, I checked the ptr on crash and it's not a nullptr
			stbi_image_free(ptr);
#endif
		} else {
	...
	if (emissiveTextureIndex != -1) { //The material has a texture...
	...
	} else { //The material has no texture
		ptl::string materialName = GetNameForMaterial(materialIndex);
