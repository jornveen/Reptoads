...
*/
	...
	var assets = new UI.Row();
	assets.add( new UI.Text( "Assets" ).setWidth( '90px' ).setMarginTop('16px') );
	container.add( assets );

	var assetList = new UI.Row( editor );
	const fs = require('fs');
	const path = require('path');
	try{
	var path_ = path.resolve(editor.config.getKey("project_folder"));
	var files = fs.readdirSync(path_,{withFileTypes:true});
	files.forEach(element => {
		if(element.isFile() && element.name.includes(".glb")){
		let btn = new UI.Button( element.name );
		btn.onClick((event)=>{
			...
			signals.assetLoad.dispatch();
			let file = new File([toArrayBuffer(fs.readFileSync(path_+"/"+btn.dom.innerHTML))],btn.dom.innerHTML);
